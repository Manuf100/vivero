...
// Validar monto y tipo de decremento
if ($monto <= 0) {
...
if ($tipo !== 'porcentaje' && $tipo !== 'valor fijo') {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Tipo de reducción inválido']);
    exit;
}

// Preparar y ejecutar la consulta SQL según el tipo de reducción
if ($tipo === 'porcentaje') {
...
if ($stmt->execute()) {
    echo json_encode(['success' => true, 'message' => 'Precios reducidos exitosamente']);
} else {
    http_response_code(500);
